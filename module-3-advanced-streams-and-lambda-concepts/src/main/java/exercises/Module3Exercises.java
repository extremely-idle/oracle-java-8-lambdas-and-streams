...
import exercises.util.RandomWords;
...
import static exercises.util.Levenshtein.lev;
...
public class Module3Exercises {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        IntStream intStream = range(0, LIST_SIZE);
    ...
    static List<String> processWords(List<String> wordList, boolean parallel) {
        final Stream<String> stream = wordList.stream();

        if (parallel) {
            stream.parallel();
        }

        final List<String> list = stream.map(String::toLowerCase)
